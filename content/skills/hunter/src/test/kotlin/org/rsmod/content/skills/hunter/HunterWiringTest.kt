...
class HunterWiringTest {
    ...
    /** The same invariant, seen from the boot path: all twelve scripts share one bus at startup. */
    @Test
    fun `all twelve scripts start together on one bus without a duplicate key`() {
        val bus = Wiring().start(*scripts.all.toTypedArray())
    ...
    /**
     * The twelve scripts, built over the same worlds the rest of the suite uses.
     *
