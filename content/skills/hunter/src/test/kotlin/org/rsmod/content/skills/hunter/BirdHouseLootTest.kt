...
/**
 * What comes out of a harvested bird house, with the dice written down.
 *
 * The rates themselves are checked against published figures in [BirdHouseNestsTest]; this checks
 * the *shape* of the payout - how many rolls are made, in what order, and what each outcome does -
 * which is the half a rate model cannot see. The draws are scripted through [ScriptedDrawRandom]
 * because a bird house draws at least a dozen times per harvest, and the ordering between those
 * draws is itself load-bearing: a clue pre-roll that ran after the type table instead of before it
 * would shift every ring and empty rate by about 2.7% and no rate test would notice.
 *
 * **The draw order, once, so every test below can be read:** one int for the feather quantity, one
 * double for the seed nest, then ten doubles for the nest rolls - each successful one taking up to
 * five ints for the clue pre-roll and then one int for the type table.
 */
...
class BirdHouseLootTest {
    ...
    /**
     * A clue replaces that roll's ring/egg/empty outcome rather than adding to it.
     *
     * "If no clue is given **then** a ring, egg, or empty nest will be awarded" - so a roll that gave
     * a clue gives nothing else, which is what keeps the other rates honest.
     *
     * What arrives is a **scroll box**, not a clue nest, because this server's default quest policy
     * is `assume-completed` and *X Marks the Spot* therefore reads as done: "clue nests are replaced
     * with one scroll box and one empty nest. Note that the empty nest drops on the ground instead of
     * going to the inventory." Both halves of that are asserted, including the floor.
     */
    @Test
