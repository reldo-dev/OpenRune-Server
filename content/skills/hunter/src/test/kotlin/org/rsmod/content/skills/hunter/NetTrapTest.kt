...
/**
 * The net trap, the only family that owns two tiles. Three invariants a live client cannot
 * exercise: the young tree must never be deleted ([a deleted young tree really does disappear] is
 * the control); the failure branch drops the rope and net on the ground, reachable only with a
 * scripted draw; and the two locs must stay paired in the *world*, not just in the arithmetic
 * ([NetTrapLogicTest]). Serialised for the reason given on [HunterTrapTickTest].
 */
...
class NetTrapTest {
    ...
    /**
     * An occupied net tile refuses the set outright (docs/hunter.md), and refusing has to be free -
     * hence the inventory assertion.
     */
    @Test
    ...
    /**
     * "If not successful, the tree will snap back to its original position, and the small fishing
     * net and rope will appear on the ground" (wiki, *Net trap*). Both halves asserted, plus the
     * consequence: the controller ends.
     */
    @Test
    ...
    /** The drop lands on the *trap's* tile, not the player's (docs/hunter.md). */
    @Test
