...
/**
 * The two rate models, checked against published figures rather than against themselves.
 *
 * This is the assertion that has real power here. Comparing [BirdHouseNests] to a table of
 * per-level probabilities generated from [BirdHouseNests] would be circular and would pass with the
 * formula transposed; every oracle below comes from **outside the code**:
 *
 * 1. **Mod Ash, 5 June 2019** (quoted on *Bird house trapping > Loot*): "You get 1 roll for a seed
 *    nest, with a chance of 0% at level 1 and 78.5% at level 99. Then you get 10 rolls for more
 *    nests. Their chances at level 99 are 10.0%, 12.5%, 12.8%, 13.0%, 14.0%, 15.0%, 16.0%, 17.0% and
 *    17.5% for the wood tiers. These scale down by half at level 50."
 * 2. **The chart template's own parameters**, read out of the snapshot's Parsoid `data-mw`: the seed
 *    nest is `low = 0, high = 200`, and the ten-roll chart carries `high1..high9` and `req1..req9`.
 * 3. **The Money Making Guide's published per-run outputs**, which are a different page written by
 *    different people and which exercise the *whole* chain - ten rolls, the per-tier rate, and the
 *    nest-type table with a rabbit foot - in one number each.
 *
