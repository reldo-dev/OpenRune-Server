...
/**
 * The magic box, which reuses the portable path wholesale and needed no new engine code.
 *
 * That is exactly why it is worth testing: "it reuses the existing path" is a claim about
 * [TrapFamily.MAGICBOX] resolving to loc names that exist and to the right ones, and nothing in the
 * type system checks it. Filing the imp under [TrapFamily.BOX] instead would have compiled, packed
 * and then thrown at the first catch, because the box trap builds its state names from its npc's
 * own name (`npc.hunting_chinchompa_big` -> `loc.hunting_boxtrap_full_chinchompa_big`) and
...
class MagicBoxTest {
    ...
    /**
     * [magicBox] with an imp beside it.
     *
     * [hunterLvl] defaults to 99, but note the imp's rate there is only 198/256 - unlike the
     * creatures whose `high` exceeds 256, no level makes its catch certain - so a catch needs
     * [ScriptedRandom.ALWAYS_CATCH] and a miss needs the requirement level plus
