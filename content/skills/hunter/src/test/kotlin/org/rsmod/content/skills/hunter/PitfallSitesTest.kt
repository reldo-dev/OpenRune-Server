...
/**
 * The twenty-five sites, checked against the packed cache rather than against themselves: every
 * hand-transcribed column fails silently in play, so the cache is asked instead - the base loc's
 * own `multiVarBit`, the companion `_animal` loc's, the child chains, and the packed map's own
 * placements read the way `GameMapDecoder` reads them. Serialised like the rest of the suite.
 */
