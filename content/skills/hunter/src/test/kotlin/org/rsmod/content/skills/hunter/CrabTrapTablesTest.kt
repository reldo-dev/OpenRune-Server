...
/**
 * The packed crab-trapping data, checked against sources that are not the table.
 *
 * Every assertion here compares the shipped row to something derived independently - the cache's own
 * `skill_features` rows, the `multiLoc` arrays the client renders from, the recolour tables that
 * pair a crab obj to its trap loc - rather than to a number retyped from the same place the table
 * got it. A test that only compares code to code enshrines whatever the author believed.
 *
...
class CrabTrapTablesTest {
    ...
    /**
     * The rainbow crab's three colourways are one creature seen three ways, and both parallel
     * columns list them in the same order.
     *
     * The three objs are the same crab: same name, same examine, same cost, and the same two
     * processing params - `param_2421` (the raw meat a knife yields) and `param_2422` (the paste a
     * pestle yields). If they were three creatures those would differ, the way the red, blue and
     * rainbow rows' do.
     *
     * The a/b/c *pairing* is a recolour-table identity - `rainbow_crab_b` (obj 31680) and
     * `crab_trap_full_rainbow_b` (loc 58911) share all thirteen recolour pairs, ending
     * `recol13s=22720 recol13d=4578` - and that is checked against the **dump**, not here: the
     * server pack drops recolour data entirely, so a packed `ObjectServerType` carries no colours to
     * compare. What is checkable here is that the two columns are index-aligned in ascending id
     * order, which is the order the recolour identity holds in. A table that listed one column
     * a/b/c and the other c/b/a would hand the player a crab whose colour does not match the trap
     * they just emptied, and this is the assertion that would catch it.
     */
    @Test
