...
/**
 * The authored trail geometry, one entry per AREA (not per varp: two varps are each split between
 * two creatures). This file is the only place that knows a coordinate and a wrong one is silent
 * in play, so all 119 are verified against sources outside this file - the local `hunterVerify`
 * sweep and the booted-world integration test; neither runs in `assemble`. Only placed segments
 * ship. The junction-derivation rule and its two prior-art cross-checks: docs/hunter.md.
 */
