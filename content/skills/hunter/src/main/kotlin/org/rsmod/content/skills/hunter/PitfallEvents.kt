...
/**
 * Pitfall trapping's player-facing ops, its cycle hook and its login rebuild.
 *
 * **Every op registered here already exists on the cache type; none is invented.** Read off the
 * packed children of the twenty-five `hunting_pitfall_<n>` locs:
 *
 * | state | child | ops |
 * |---|---|---|
 * | 0 [PitState.Empty] | `hunting_pitfall_invis_empty` | `op3=Trap` |
 * | 1 [PitState.Set] | `hunting_pitfall_invis_set` | `op1=Jump`, `op2=Dismantle` |
 * | 2 [PitState.Catching] | `hunting_pitfall_invis_catching` | *(none)* |
 * | 3/4 [PitState.Full] | `hunter_pitfall_full[_<cat>[_180]]` | `op2=Dismantle` |
 *
 * **None of them is the loc the map placed, and none of them is a content group.** The twenty-five
 * sites are `multiloc` parents carrying no ops of their own, and `LocInteractions.opTrigger`
 * resolves the child *before* it looks for a handler - the shape [CrabTrapEvents] and
 * [BirdHouseEvents] both document. Those two answered it with a content group because they had
 * nine and twenty-eight children to cover; a pitfall has **nine**, so nine direct registrations
 * beat a `[gamevals.content]` id plus nine `contentGroup` blocks in
 * `.data/raw-cache/server/loc/loc.toml` - the two-declaration rule nine times over, for no gain.
 * The handlers still read the site off `loc` (the map-placed parent) and its state off the
 * **varbit**, never off `vis`: the varbit is what the client rendered from and what the server is
 * about to overwrite, so the two cannot disagree.
 *
 * The four `hunter_pitfall_full_antelope_*` locs are deliberately absent. They render the antelope
 * beside the pit from the companion `hunting_pitfall_<n>_animal` loc, are `active=no`, and carry no
 * ops - so is `hunting_pitfall_invis_catching`, and so, effectively, is
 * `hunting_pitfall_invis_collpased`, which declares `op2` but sits in no multiloc chain at this
 * revision. See [PitfallSites.opLessLocs].
 *
 * `onAiConTimer` is registered nowhere here. A pitfall has no controller: nothing of it lives in
 * the world at all - it is per-player varbit state written onto permanent map scenery.
 */
